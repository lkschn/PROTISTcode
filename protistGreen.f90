...
!                                                                                                     
	...
	do iseg = 1 , noseg
		...
		do iSpec = 0, (nrSp-1)
            ...
            !write(81,*) protC, Cfix, totR, mrt, iseg   
            !write(67,*) protC
            if ( isnan(protC) ) write ( 60, '(''ERROR: NaN in protC in segment:'', i10)' )    iseg
            if ( isnan(Cfix) )  write ( 61, '(''ERROR: NaN in Cfix in segment:'', i10)' )    iseg
            if ( isnan(totR) )  write ( 62, '(''ERROR: NaN in totR in segment:'', i10)' )    iseg
            if ( isnan(mrt) )   write ( 63, '(''ERROR: NaN in mrt in segment:'', i10)' )    iseg
            if ( isnan(NC) )    write ( 64, '(''ERROR: NaN in NC in segment:'', i10)' )    iseg
            if ( isnan(PC) )    write ( 65, '(''ERROR: NaN in PC in segment:'', i10)' )    iseg
            if ( isnan(ChlC) )  write ( 66, '(''ERROR: NaN in ChlC in segment:'', i10)' )    iseg
