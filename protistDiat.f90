...
!                                                                                                     
	...
	do iseg = 1 , noseg
		...
		do iSpec = 0, (nrSp-1)
            ...
            !write(80,*) protC, Cfix, totR, mrt, iseg
            !write(58,*) protC
                        
            if ( isnan(protC) ) write ( 50, '(''ERROR: NaN in protC in segment:'', i10)' )    iseg
            if ( isnan(Cfix) )  write ( 51, '(''ERROR: NaN in Cfix in segment:'', i10)' )    iseg
            if ( isnan(totR) )  write ( 52, '(''ERROR: NaN in totR in segment:'', i10)' )    iseg
            if ( isnan(mrt) )   write ( 53, '(''ERROR: NaN in mrt in segment:'', i10)' )    iseg
            if ( isnan(NC) )    write ( 54, '(''ERROR: NaN in NC in segment:'', i10)' )    iseg
            if ( isnan(PC) )    write ( 55, '(''ERROR: NaN in PC in segment:'', i10)' )    iseg
            if ( isnan(ChlC) )  write ( 56, '(''ERROR: NaN in ChlC in segment:'', i10)' )    iseg
            if ( isnan(SC) )    write ( 57, '(''ERROR: NaN in SC in segment:'', i10)' )    iseg
